...
idCVar r_filmgrain("r_filmgrain", "0", CVAR_RENDERER | CVAR_INTEGER | CVAR_ARCHIVE, "1 - 5, log2 of the amounght of grain on image");
idCVar r_postfx("r_postfx", "1", CVAR_RENDERER | CVAR_INTEGER | CVAR_ARCHIVE, "1 - 5, 0 - disable render post prossessing, 1 - all enable, 2 < disable specific FXs");
	...
	if ( pStage->texture.texgen == TG_REFLECT_CUBE ) 
	{
		...
		else if (tr.backEndRenderer == BE_GLSL)
		{
			...
			if (bumpStage != NULL)
			{
				...
				{
					common->Printf("TexGen: TG_REFLECT_CUBE: Bumpy Environment\n");
				}

				DrawGLSL.R_DrawShader(BUILTIN_BUMP_ENVIRONMENT);

				DrawGLSL.R_GetRenderProgram(BUILTIN_BUMP_ENVIRONMENT).SetAttrbLocation("attr_Tangent", 9);
				DrawGLSL.R_GetRenderProgram(BUILTIN_BUMP_ENVIRONMENT).SetAttrbLocation("attr_Bitangent", 10);
				...
				glVertexAttribPointer(9, 3, GL_FLOAT, false, sizeof(idDrawVert), ac->tangents[1].ToFloatPtr());
				glVertexAttribPointer(10, 3, GL_FLOAT, false, sizeof(idDrawVert), ac->tangents[0].ToFloatPtr());
			}
			else
			{
				if (r_debugContext.GetInteger() >= 3)
				{
					common->Printf("TexGen: TG_REFLECT_CUBE: Environment\n");
				}
				DrawGLSL.R_DrawShader(BUILTIN_ENVIRONMENT);
			}
#if  0
			glDisableVertexAttribArray(VertexTangentID);//<--
			glDisableVertexAttribArray(VertexBitangentID);//<-- eye Here
#else
			glDisableVertexAttribArray(9);//<--
			glDisableVertexAttribArray(10);//<-- eye Here
#endif
	...
	if ( pStage->texture.texgen == TG_REFLECT_CUBE ) 
	{
		...
		if (tr.backEndRenderer == BE_GLSL)
		{
			...
			if (bumpStage)
			{
				...
				globalImages->BindNull();
				GL_SelectTexture(0);
				DrawGLSL.R_DeleteShader(BUILTIN_BUMP_ENVIRONMENT);
			}
			else
			{
				DrawGLSL.R_DeleteShader(BUILTIN_ENVIRONMENT);
			}
			glUseProgram(0);
		}
#endif
...
static void RB_T_Shadow( const drawSurf_t *surf ) {
	...
	if ( tr.backEndRendererHasVertexPrograms && r_useShadowVertexProgram.GetBool()
		...
		/*NMS Begins: Draw Stencil shadow using GLSL*/
		else if (tr.backEndRenderer == BE_GLSL) 
		{
			DrawGLSL.RB_GLSL_DrawShadownInteration(1, localLight);
		}
		/*NMS END*/
	}
...
static void RB_BlendLight( const drawSurf_t *drawSurfs,  const drawSurf_t *drawSurfs2 ) {
	...
	/*NMS Begin*/
#if 0 //Back in future build
	//const idMaterial	*AmbientShader;
	//AmbientShader = backEnd.vLight->ambientShader;

	// texture 2 will get the AmbientCube texture
	GL_SelectTexture(2);
	glDisableClientState(GL_TEXTURE_COORD_ARRAY);
	glEnable(GL_TEXTURE_GEN_S);
	//glTexCoord2f(0, 0.5);
	backEnd.vLight->ambientImage->Bind();
#endif
	/*NMS End*/
...
void RB_DoPostProssecing()
{
	if (r_postfx.GetInteger() <= 0) 
	{
		return;
	}
	...
	globalImages->currentRenderImage->CopyFramebuffer(	backEnd.viewDef->viewport.x1,
														...
														true);
	//copy the depth
	globalImages->currentRenderDepth->CopyDepthbuffer(	backEnd.viewDef->viewport.x1,
														backEnd.viewDef->viewport.y1,
	...
	idImage * image = globalImages->images[2];
	...
	RenderProgram GrainProgram = DrawGLSL.R_GetRenderProgram(BUILTIN_FILMGRAIN);

	GrainProgram.UseProgram();

	float Width = glConfig.vidWidth;
	float Height = glConfig.vidHeight;
	float timer = Sys_Milliseconds();

	glUniform1f(GrainProgram.GetUniform(UN_SCREENSIZE_WIDH), Width);
	glUniform1f(GrainProgram.GetUniform(UN_SCREENSIZE_HEIG), Height);
	glUniform1f(GrainProgram.GetUniform(UN_TIMER), timer);

	GL_SelectTexture(0);
	image->Bind();

	//GlobalFrameBuffers.DrawScreenRect();
...
void	RB_STD_DrawView(void)
{
	...
	if (backEnd.viewDef->viewEntitys != NULL && r_postfx.GetInteger() > 0)
	{
		GlobalFrameBuffers.RenderBuffer->Bind(nmFrameBuffer::TO_BIND);
	}
#endif
	...
	case BE_GLSL:
		DrawGLSL.RB_GLSL_DrawInteractions();
		break;
	}
