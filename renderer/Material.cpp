...
idImage *idMaterial::GetEditorImage( void ) const {
	...
	if ( !editorImageName.Length()) {
		// _BMbase :: First check for a diffuse image, then use the first
		if ( numStages && stages ) {
			int i;
			for( i = 0; i < numStages; i++ ) {
...
void idMaterial::ParseStage( idLexer &src, const textureRepeat_t trpDefault ) {
	...
	while ( 1 ) {
		...
		if ( !token.Icmp( "program" ) ) {
			if ( src.ReadTokenOnLine( &token ) ) {
				...
				if (tr.backEndRenderer == BE_GLSL)
				{
					newStage.vertexProgram = DrawGLSL.R_FindGLSLShader(token.c_str(), GL_VERTEX_SHADER);
					newStage.fragmentProgram = DrawGLSL.R_FindGLSLShader(token.c_str(), GL_FRAGMENT_SHADER);
				}
				else
				{
		...
		if ( !token.Icmp( "megaTexture" ) ) {
			if ( src.ReadTokenOnLine( &token ) ) {
				...
				if (tr.backEndRenderer == BE_GLSL)
				{
					newStage.vertexProgram = DrawGLSL.R_FindGLSLShader("megaTexture.vfp", GL_VERTEX_SHADER);
					newStage.fragmentProgram = DrawGLSL.R_FindGLSLShader("megaTexture.vfp", GL_FRAGMENT_SHADER);
				}
				else
				{
	...
	/*NMS Begin: Create a shader Program Based on RBDoomBFG*/
	if (tr.backEndRenderer == BE_GLSL)
	{
		// if we are using newStage, allocate a copy of it
		if (newStage.fragmentProgram || newStage.vertexProgram)
		{
			newStage.glslProgram = DrawGLSL.R_FindGLSLProgram(GetName(), newStage.vertexProgram, newStage.fragmentProgram);
...
void idMaterial::ParseMaterial( idLexer &src ) {
	...
	while ( 1 ) {
		...
		else if (!token.Icmp("AmbientCubeMap"))
		{
			str = R_ParsePastImageProgram(src);
			idStr	copy;

			copy = str;	// so other things don't step on it
			AmbientMapImage = globalImages->ImageFromFile(copy, TF_DEFAULT, false, TR_CLAMP, TD_DEFAULT, CF_NATIVE);
		}
