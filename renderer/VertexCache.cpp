...
void idVertexCache::ActuallyFree( vertCache_t *block ) {
	...
	if ( block->tag != TAG_TEMP ) {
		...
		if ( block->vbo ) {
			...
			glBindBuffer(GL_ARRAY_BUFFER, 0, 0, GL_DYNAMIC_DRAW);
#endif
...
The _vertex_buffer_object will be bound
==============
*/
void *idVertexCache::Position( vertCache_t *buffer ) {
	...
	if ( buffer->vbo ) {
		...
		//Now use the GLEW Vertex Buffer
...
void idVertexCache::Init() {
	...
	// use _vertex_buffer_object unless explicitly disabled
	if( r_useVertexBuffers.GetInteger() && glConfig.VertexBufferObjectAvailable ) {
		common->Printf( "using vertex_buffer_object memory\n" );
	} else {
		virtualMemory = true;
...
void idVertexCache::List( void ) {
	...
	if ( !virtualMemory  ) {
		common->Printf( "Vertex cache is in _vertex_buffer_object memory (FAST).\n");
	} else {
