...
#include "renderer/drawprogs/FrameBuffer.h"
...
const char *r_rendererArgs[] = { "best", "glsl", "arb2", NULL };
...
//NMS Begin: Multiversion GLSL Suport
const char *r_GLSLArgs[] = { "GLSL4", "GLSL3", "GLSL1.2", NULL };
idCVar r_GLSLVer("r_GLSLVer", "GLSL3", CVAR_RENDERER | CVAR_ARCHIVE, "hardware specific GLSL shading version to use", r_GLSLArgs, idCmdSystem::ArgCompletion_String<r_GLSLArgs>);
...
idCVar r_debugContext("r_debugContext", "0", CVAR_RENDERER |CVAR_ARCHIVE, "Enable various levels of context debug.");
...
//Enable use of Half-Life2 - Half-Lambertian Lighthing model
idCVar r_useHalfLambert("r_useHalfLambert", "1", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "Enable use of Half-Lambert lighthing");
idCVar r_useRimLight("r_useRimLight", "1", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "Enable use of Rim lighthing, the light of the edges of the model");
idCVar r_skipMaterialMask("r_skipMaterialMask", "0", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "skip all masks options");

//NMS End

/*NMS Begin: Based on Work of RBDoom3BFG*/
/*
========================
DebugCallback
...
static void R_CheckPortableExtensions( void )
{
	...
	glConfig.glSlVersion = atof(glConfig.GLSL_version_string);
	...
	//NMS Begin
	// GLSL Shading suport
	if (glConfig.glSlVersion >= 1.20) //minimum requerided GLSL ver 1.20
	{
		glConfig.GLSLAvailable = true;
		common->Printf("GLSL %f avaidable\n", glConfig.glSlVersion);

		//check the maximum GL attribs suport
		glGetIntegerv(GL_MAX_VERTEX_ATTRIBS, (GLint *)&glConfig.maxVertexAttribs);

		//minimum requerided vertexes atribs are 4
		if (glConfig.maxVertexAttribs < 4)
			common->Warning("Max vertexes attribs %d, requerided are 4", glConfig.maxVertexAttribs);
		else
			common->Printf("Maximum number of vertex attributes supported: %d \n", glConfig.maxVertexAttribs);
	}
	else
	{
		glConfig.GLSLAvailable = false;
		common->Warning("No avaidable GLSL suport\n");
	}

	glConfig.directStateAccess = R_CheckExtension("GL_EXT_direct_state_access");

	glConfig.sRGBTextureAvailable = R_CheckExtension("GL_EXT_texture_sRGB");

	//Render to Frame buffer
	//GL_EXT_framebuffer_object
	glConfig.framebufferAvailable = R_CheckExtension("GL_EXT_framebuffer_object");
	if (glConfig.framebufferAvailable)
	{
...
# if TODO
	//Tesselation
	//NMS Begin: If have openGL4 and suport Tesselation
	if (glConfig.glVersion >= 4.0f)
	{
		glConfig.tessellationAvailable = R_CheckExtension("GL_ARB_tessellation_shader");
		if (glConfig.tessellationAvailable)	idCVar r_enableTesselation("r_enableTesselation", "0", CVAR_RENDERER | CVAR_BOOL, "Enable Tesselation");
	}
#	endif
...
void R_InitOpenGL( void ) 
{
	...
	//NMS Begin
	glConfig.GLSL_version_string = (const char*)glGetString(GL_SHADING_LANGUAGE_VERSION);
	//NMS end
	...
	//NMS Begin
	common->Printf("OpenGL GLSL version: %s\n", glConfig.GLSL_version_string);
	//NMS  End
	...
	//NMS Begin: GL Shading Language suport
	//Init GLSL Sistem an load all build-ins programs
	DrawGLSL.R_GLSL_Init();
	//NMS End
	...
	//NMS Begin
#if 0
	GlobalFrameBuffers.CreateScreen();
#endif
	//NMS End
...
void idRenderSystemLocal::TakeScreenshot( int width, int height, const char *fileName, int blends, renderView_t *ref ) {
	...
	// _BMbase adds viewnote screenie save to cdpath
...
void idRenderSystemLocal::InitOpenGL( void ) {
	...
	//NMS Begin
	//init all frame buffers
#if NMS_FRAME_BUFFER
	GlobalFrameBuffers.init();
#endif
	//NMS End
}
