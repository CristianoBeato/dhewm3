...
bool idRenderWorldLocal::Trace( modelTrace_t &trace, const idVec3 &start, const idVec3 &end, const float radius, bool skipDynamic, bool skipPlayer /*_BMbase*/ ) const {
	areaReference_t * ref;
	idRenderEntityLocal *def;
	portalArea_t * area;
	idRenderModel * model;
...
#if 1	/* _BMbase addition. could use a cleaner approach */
				if ( skipPlayer ) {
...
============
*/
void idRenderWorldLocal::DebugSphere( const idVec4 &color, const idSphere &sphere, const int lifetime, const bool depthTest /*_BMbase*/ ) {
	int i, j, n, num;
	float s, c;
