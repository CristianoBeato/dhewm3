...
#include "renderer/imagefiles/Image.h"
...
{
	//NMS Begin: SRGB Texture Colors
#if NMS_SRGB_SUPORT_TOFIX
	int sRGB = r_useSRGB.GetInteger();
	if (glConfig.sRGBTextureAvailable == true && (sRGB == 1 || sRGB == 3))
		if (internalFormat < GL_COMPRESSED_SRGB_S3TC_DXT1_EXT || internalFormat > GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT ||
			internalFormat < GL_COMPRESSED_RGB_S3TC_DXT1_EXT || internalFormat > GL_COMPRESSED_RGBA_S3TC_DXT5_EXT) return false;
	else
		if ( internalFormat < GL_COMPRESSED_RGB_S3TC_DXT1_EXT|| internalFormat > GL_COMPRESSED_RGBA_S3TC_DXT5_EXT ) return false;
#else
	if (internalFormat < GL_COMPRESSED_RGB_S3TC_DXT1_EXT || internalFormat > GL_COMPRESSED_RGBA_S3TC_DXT5_EXT) return false;
#endif
	//NMS End
...
int idImage::BitsForInternalFormat( int internalFormat ) const 
{
	...
	case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
		return 8;
//NMS Begin Suport To SRGB Color space 
#if NMS_SRGB_SUPORT
...
===============
*/
GLenum idImage::SelectInternalFormat(const byte **dataPtrs, int numDataPtrs,
	...
	textureDepth_t minimumDepth) const
{
...
#else
#if NMS_SRGB_SUPORT //SRGBALPHA Color Supot
...
#else
		// we need the specular map alpha chanel as gloss map 
		if (glConfig.textureCompressionAvailable)
	...
	if (minimumDepth == TD_DIFFUSE) {
		...
		{
			if (glConfig.textureCompressionAvailable)
				if (!needAlpha)							return GL_COMPRESSED_SRGB_S3TC_DXT1_EXT;
				else									return GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT;

			else if ((aAnd == 255 || aOr == 0))			return GL_SRGB8;
			else										return GL_SRGB8_ALPHA8;
		}
		else
		{
			if (glConfig.textureCompressionAvailable)
				if (!needAlpha)					return GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
				else							return GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;

			else if ((aAnd == 255 || aOr == 0))	return GL_RGB5;
			else								return GL_RGBA4;
		}
	...
	//Use only for frame Buffer Texture Only
	if (minimumDepth == TD_FRAME_BUFFER_HIGH)
		return	GL_RGB32F; //maybe is tool much but 

	if (minimumDepth == TD_FRAME_BUFFER_LOW)
		return	GL_RGB16F; //i need say something... but dont now what
#endif
	...
	if (glConfig.sRGBTextureAvailable == true && (sRGB == 1 || sRGB == 3))
	{
		...
		if (!needAlpha)
		{
			if (minimumDepth == TD_HIGH_QUALITY)		return GL_SRGB8;
			if (glConfig.textureCompressionAvailable)	return GL_COMPRESSED_SRGB_S3TC_DXT1_EXT;
			return GL_RGB8;
		}
	...
	if (glConfig.sRGBTextureAvailable == true && (sRGB == 1 || sRGB == 3))
	{
		if (minimumDepth == TD_HIGH_QUALITY)			return GL_SRGB8_ALPHA8;
		if (glConfig.textureCompressionAvailable)		return GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT;
		if (!rgbDiffer)									return GL_SLUMINANCE8_ALPHA8;
		return GL_SRGB_ALPHA;
	}
	else
	{
		if (minimumDepth == TD_HIGH_QUALITY)			return GL_RGBA8;	// four bytes
		if ( glConfig.textureCompressionAvailable )		return GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;	// one byte
		if ( !rgbDiffer )								return GL_LUMINANCE8_ALPHA8;	// two bytes, max quality
...
void idImage::GenerateImage( const byte *pic, int width, int height,
	...
	else if (internalFormat == GL_DEPTH_COMPONENT)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, scaled_width, scaled_height, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, scaledBuffer);
	}
	//NMS End
	else 
	{
...
void idImage::WritePrecompressedImage() {
	...
	if ( FormatIsDXT( altInternalFormat ) ) {
		...
		if (glConfig.sRGBTextureAvailable == true && (sRGB == 1 || sRGB == 3))
			...
			(altInternalFormat <= GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT ? 8 : 16);
		else
			header.dwPitchOrLinearSize = ((uploadWidth + 3) / 4) * ((uploadHeight + 3) / 4)*
	...
	for ( int level = 0 ; level < numLevels ; level++ ) {
		...
		if ( FormatIsDXT( altInternalFormat ) ) 
		{
			...
			if (glConfig.sRGBTextureAvailable == true && (sRGB == 1 || sRGB == 3))
				...
				size = ( ( uw + 3 ) / 4 ) * ( ( uh + 3 ) / 4 ) * (altInternalFormat <= GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT ? 8 : 16);
#else
		if (FormatIsDXT(altInternalFormat)) {
			size = ((uw + 3) / 4) * ((uh + 3) / 4) *
				(altInternalFormat <= GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ? 8 : 16);
...
void idImage::UploadPrecompressedImage( byte *data, int len ) {
	...
	for ( int i = 0 ; i < numMipmaps; i++ ) {
		...
		if ( FormatIsDXT( internalFormat ) )
		{
			...
			if (glConfig.sRGBTextureAvailable == true && (sRGB == 1 || sRGB == 3))
			{
				//size = ((uw + 3) / 4) * ((uh + 3) / 4) * (internalFormat <= GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT ? 8 : 16);
				if (internalFormat == GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT || internalFormat == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT)
					size = ((uw + 3) / 4) * ((uh + 3) / 4) * 8;
				else
					size = ((uw + 3) / 4) * ((uh + 3) / 4) * 16;
			}
			else
			{
...
void idImage::CopyFramebuffer( int x, int y, int imageWidth, int imageHeight, bool useOversizedBuffer ) {
	...
	if ( ( useOversizedBuffer && ( uploadWidth < potWidth || uploadHeight < potHeight ) )
		...
		if ( potWidth == imageWidth && potHeight == imageHeight ) 
		{
			...
			glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB12, x, y, imageWidth, imageHeight, 0);
		} else {
			byte	*junk;
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB12, potWidth, potHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, junk);
			Mem_Free( junk );
