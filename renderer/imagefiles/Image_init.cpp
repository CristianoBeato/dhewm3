...
//NMS Begin
static void R_DephtImage(idImage *image)
{
#if 0
	byte	data[DEFAULT_SIZE][DEFAULT_SIZE][4];
	memset( data, 0, sizeof( data ) );
...
#endif
	image->GenerateImage( (byte *)data,DEFAULT_SIZE, DEFAULT_SIZE, TF_DEFAULT, false, TR_REPEAT, TD_DEPTH );
}
...
#if 0
	image->GenerateImage(NULL, glConfig.vidWidth, glConfig.vidHeight, TF_NEAREST, false, TR_CLAMP, TD_FRAME_BUFFER_LOW);
#else
	image->GenerateImage(NULL, DEFAULT_SIZE, DEFAULT_SIZE, TF_NEAREST, false, TR_CLAMP, TD_FRAME_BUFFER_LOW);
#endif
}

static void R_RGBA32FImage(idImage* image)
{
	image->GenerateImage(NULL, glConfig.vidWidth, glConfig.vidHeight, TF_NEAREST, false,TR_CLAMP, TD_FRAME_BUFFER_HIGH);
}
#endif
//NMS End
...
void idImageManager::Init() {
	...
	currentRenderDepth = ImageFromFunction("_currentRenderDepth", R_DephtImage);

	//NMS Begin
#if NMS_FRAME_BUFFER_TEXTURE
#if 0
	FirstPostFXPASS_COLOR = ImageFromFunction("_currentRenderC", R_RGBA16FImage);
	FirstPostFXPASS_NORMAL = ImageFromFunction("_currentRenderN", R_RGBA16FImage);
	FirstPostFXPASS_SPECULAR = ImageFromFunction("_currentRenderS", R_RGBA16FImage);
#else
	FirstPostFXPASS_COLOR = ImageFromFunction("_currentRenderC", R_RGBA8Image);
	FirstPostFXPASS_NORMAL = ImageFromFunction("_currentRenderN", R_RGBA8Image);
	FirstPostFXPASS_SPECULAR = ImageFromFunction("_currentRenderS", R_RGBA8Image);
#endif
#endif
	//NMS End
