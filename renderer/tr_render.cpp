...
#if 0
	//NMS Begin
	if (!din->materialMask || r_skipMaterialMask.GetBool())
	{
		din->materialMask = globalImages->blackImage;
	}
	//endif
#endif
...
#if 0
				//NMS Begin
				case SL_MATERIALMASK: 
				{
					// ignore stage that fails the condition
					if (!surfaceRegs[surfaceStage->conditionRegister]) {
						break;
					}
					if (inter.materialMask) {
						RB_SubmittInteraction(&inter, DrawInteraction);
					}
					R_SetDrawInteraction(surfaceStage, surfaceRegs, &inter.materialMask, inter.specularMatrix, NULL);
					break;
				}
				//NMS End
#endif
...
void RB_DrawView( const void *data ) {
	...
	//Bind the Geometry Frame Buffer
#if 0
	if (backEnd.viewDef->viewEntitys)
	{
		GlobalFrameBuffers.RenderBuffer->BindForReading();
	}
#endif
